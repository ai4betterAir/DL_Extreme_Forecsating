...
class Sparse_LSTM_Class(object):
    MODEL_DISPLAY_NAME = "Sparse_LSTM_v2"
    MODEL_TAG = "v2"
    DEFAULT_MODEL_NAME = "sparse_lstm_v2"
    """
...
###########################################################################################
    def _format_output(self, predictions, station_names,
        ...
        """
        list_output_pd = []
        forecast_horizon = getattr(self.Configuration, "n_steps_out", None)
        if isinstance(forecast_horizon, int) and forecast_horizon > 0 and predictions.shape[0] > forecast_horizon:
            self.logger.info(
                f"  trimming rolling forecast to the latest {forecast_horizon} step(s)".ljust(
                    self.justif - 2, '.') + 'OK'
            )
        ...
        if start_timestamp is None:
            ...
            # `start_timestamp` is the last observed timestamp; forecast rows
            # are labelled from the next hour onward.
            timestamps = pd.date_range(
...
if __name__ == '__main__':
    ...
    justif     = 102
    loggername = 'Sparse_LSTM_v2_Test'
    ...
    logger.info('Sparse_LSTM_v2 standalone test complete')

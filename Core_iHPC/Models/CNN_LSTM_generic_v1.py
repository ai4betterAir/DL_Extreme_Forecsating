...
###########################################################################################
class CNN_LSTM_generic_Class(object):
    """ 
...
###########################################################################################
    def fit(self,
        ...
        if save_model:
            ...
            self.logger.info('CNN LSTM model save file {file}'.format(file=file).ljust(self.justif-5,'.') + 'SAVED')

        self.logger.info('CNN LSTM model training'.ljust(self.justif-8,'.') + 'COMPLETE')
...
###########################################################################################
    def configure_model(self, n_steps_in, n_steps_out, n_epochs, input_columns, output_columns, train_model ):
        ...
        if train_model:
            ...
            self.logger.info('CNN-LSTM 2D model save file {file}'.format(file=file).ljust(self.justif-6,'.') + 'LOADED')
...
###########################################################################################
    def cost_function(self,
        ...
        update_dict = {key : int(np.rint(val)) for key,val in dict_params.items()}
        # print(update_dict)
        self.Configuration.model_parameters_dict.update(update_dict)
...
###########################################################################################
    def run_all(self, input_data_pd, iterative ):
        ...
        if train_model:
            ...
            # model_parameters_optimisation = False
            ...
            list_output_pd = TE.evaluate_cnn_lstm_training_data(
        ...
        #######################################################################################
            ...
            forecast_pd, full_forecast_pd = Data_prep_class.make_forecast(datetime_filtered_input_data_ds, output_columns, datetime_filtered_input_data_pd,  self.model)

            list_output_pd = [full_forecast_pd]
            self.logger.info('Model Forecasting'.ljust(self.justif-4,'.') + 'DONE')
        
        return list_output_pd, history           
...
###########################################################################################
    def lstm_bnn_model (self,
        ...
        """
            This model used for 48-48 forecast
        """                     
        ...
        if build_architecture:
            ...
            # #### CHECK THE OUTPUT !!!!
            # self.model.add((TKL.Dense(n_steps_out*n_features_out, activation=activation)))
            # # self.model.add(TKL.Reshape((n_steps_out,n_features_out)))
            ...
            lstm =       TKL.LSTM(lstm_1_units, return_sequences=True, activation=activation_lstm, name='lstm1', bias_regularizer=regularizers.l1_l2(l1=0.0001, l2=0.0001))(input_layer)
        ...
        self.logger.info('LSTM-BNN model v2'.ljust(self.justif-10,'.') + 'CONFIGURED')

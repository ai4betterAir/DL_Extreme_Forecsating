...
###########################################################################################
    def cnn_lstm_model_v1(self,
        ...
        if build_architecture:
            ...
            self.model.add(TKL.TimeDistributed(TKL.Dense(n_features_out, activation='relu')))  
        ...
        self.logger.info('CNN LSTM model v1'.ljust(self.justif-10,'.') + 'CONFIGURED')
        return
###########################################################################################
 ###########################################################################################
    def fit(self,
...
###########################################################################################
    def configure_model(self, n_steps_in, n_steps_out, n_epochs, input_columns, output_columns, train_model ):
        ...
        if train_model:
            ...
            self.cnn_lstm_model_v1(n_steps_in, n_steps_out, n_features_in, n_features_out, build_architecture)   
...
###########################################################################################
    def run_all(self, input_data_pd, iterative ):
        ...
        if train_model:
            ...
            list_output_pd = Data_prep_class.evaluate_data(dict_split_final_datasets[which_set], output_columns, n_steps_in, n_steps_out, 
